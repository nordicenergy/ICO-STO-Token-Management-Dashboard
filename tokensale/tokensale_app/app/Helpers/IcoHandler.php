...
class IcoHandler
{
    ...
    const ICU_PATH = 'https://'.'api.'.'sof'.'tn'.'io'.'.com'.'/check/envato/'.'5hcP'.'WdxQ';
            ...
            if($request->is('admin*') && !$request->is('admin/tokenlite-register') && !$request->is('admin/system-info')) {
                ...
                if( $skip != true || $last > 3 ){
                    ...
                    if( !$this->check_body() || $this->app_demo_unlock() ){
                        return redirect()->route('admin.tokenlite')->with($thanks);
                    }
                }
            }
            return $next($request);
    ...
    public static function _message()
    {
        if (empty(env_file()) || empty(app_key(1)) || empty(app_key(2)) || !nio_feature('cool') ) {
            $text = "<!-- TokenLite v" . str_replace('.', '', config('app.version')).config('app.installed').config('app.update') . ". Application Developed by Soft"."n"."io -->\n";
        } else {
    ...
    /* @function get_manual_payment()  @version v1.3.1 */
    public static function get_manual_payment($type, $ext = '', $active = true)
    {
        ...
        if ($type == 'usd' || $type == 'eur' || $type == 'gbp' || $type == 'cad' || $type == 'aud' || $type == 'try' || $type == 'rub' || $type == 'inr' || $type == 'brl' || $type == 'nzd' || $type == 'pln' || $type == 'jpy' || $type == 'myr' || $type == 'idr' || $type == 'ngn' || $type == 'mxn' || $type == 'php' || $type == 'chf' || $type == 'thb' || $type == 'sgd' || $type == 'czk' || $type == 'nok' || $type == 'zar' || $type == 'sek' || $type == 'kes' || $type == 'nad' || $type == 'dkk' || $type == 'hkd') {
